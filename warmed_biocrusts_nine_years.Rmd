...
waterdays <- read.delim(pasteo(proj_dir, "0607wateringdays.txt"), string = F) %>% 
...
rainfall %>% 
  ...
  summarise(rain24h = sum(rain, na.rm = TRUE)) %>% View
...
with(NSE_photo.te$comparisons, mapply(function(a,b,c) 
  paste("[", b, ", ", c, "]", sep =''), round(meandiff, 1), 
  round(bootdiff_lo, 1), round(bootdiff_hi, 1))) %>% cat(., sep ='   ')
